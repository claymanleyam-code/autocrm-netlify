...
exports.handler = async function (event) {
    ...
    if (!sendRes.ok) {
          ...
          if (sheetId && row_number && status_col_letter) {
                  try { await updateCell(access_token, sheetId, tabA1(status_col_letter, row_number), 'Error'); } catch (_) {}
          }
    ...
    if (sheetId && row_number) {
          try {
                  if (status_col_letter) await updateCell(access_token, sheetId, tabA1(status_col_letter, row_number), 'Sent');
    ...
    return json(200, { ok: true, gmail_id: sendJson.id, sent_at: now });
};
