...
// Body: {
//   refresh_token, from_email, to, subject, body, attachment: {filename, mimeType, data},
...
function buildRfc822({ from, to, subject, body, attachment }) {
    const headerLines = [
          `From: ${from}`,
          `To: ${to}`,
          `Subject: ${subject}`,
  ...
  const boundary = 'mixed_' + Math.random().toString(36).slice(2) + Date.now();
    const safeName = (attachment.filename || 'attachment').replace(/"/g, '');
